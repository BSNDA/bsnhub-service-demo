...
func InitRelayerTransRecord(
	...
	common.Logger.Infof("Init Relayer trans record , requestId is %s,tx status is %d", requestId, txStatus)
	...
	if err != nil {
		common.Logger.Errorf("Init Relayer trans record Failed :%s", err.Error())
	} else {
		common.Logger.Infof("Init Relayer trans record  lastId:%d ;rows:%d ", lastId, rows)
	}
...
func RelayerResponeRecord(data *RelayerResInfo) {
	...
	common.Logger.Infof("set relayer callback record , requestId is %s,tx status is %d", data.RequestId, data.TxStatus)
	...
	if err != nil {
		common.Logger.Errorf("set relayer callback record Failed :%s", err.Error())
	} else {
		common.Logger.Infof("set relayer callback record lastId:%d ;rows:%d ", lastId, rows)
	}
...
func InitProviderTransRecord(requestId, to_chainid, ic_request_id, to_tx, error string, tx_status int) {
	...
	common.Logger.Infof("Init Provider trans record , requestId is %s,tx status is %d", requestId, tx_status)
	insertsql := fmt.Sprintf("INSERT INTO %s ( "+
		"request_id, "+
		"ic_request_id ,"+
		"to_chainid ,"+
	...
	if err != nil {
		common.Logger.Errorf("Init Provider trans record Failed :%s", err.Error())
	} else {
		common.Logger.Infof("Init Provider trans record  lastId:%d ;rows:%d ", lastId, rows)
	}
...
func ProviderCallBackTransRecord(data *ProviderResInfo) {
	common.Logger.Infof("set provider callback record , ic_requestId is %s,res txid is %s,tx status is %d", data.IcRequestId, data.HUBResTxId, data.TxStatus)
	if data.IcRequestId == "" {
		return
	}
