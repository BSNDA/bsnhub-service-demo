...
import (
	"encoding/hex"
	"encoding/json"
	"eth-contract-call-service-provider/mysql/store"
	"fmt"
	...
	"eth-contract-call-service-provider/contract-service/eth"
	...
	"eth-contract-call-service-provider/types"
)

// ContractService defines the contract service
type ContractService struct {
	EthClient *eth.EthChain
	Logger      *log.Logger
}
...
func BuildContractService(v *viper.Viper, chainManager *server.ChainManager) (ContractService, error) {
	...
	return ContractService{
		EthClient: eth.NewEthChain(*baseConfig, chainManager),
	}, nil
}
...
func (cs ContractService) Callback(reqCtxID, reqID, input string) (output string, result string) {
	...
	chainParams, err := cs.EthClient.ChainManager.GetChainParams(request.Dest.ChainID)
	if err != nil {
		res.Code = 204
	...
	err = cs.EthClient.InstantiateClient(chainParams)
	if err != nil {
		res.Code = 500
		res.Message = "failed to connect to the eth node"
	...
	auth, err := cs.EthClient.BuildAuthTransactor()
	if err != nil {
		res.Code = 500
		res.Message = "invalidate account"

		return
	}

	tx, err := cs.EthClient.TargetCoreContract.CallService(auth, requestIDByte32, contractAddress, request.CallData)
	if err != nil {
		//mysql.TxErrCollection(reqID, err.Error())
		cs.Logger.Errorf("Eth ChainId %s Chaincode %s CallService has error %v", request.Dest.ChainID, request.Dest.EndpointAddress, err)
		//不包含重复交易，再记录
	...
	receipt, err := cs.EthClient.WaitForReceipt(tx, "CallService")
	if err != nil {
		//mysql.TxErrCollection(reqID, err.Error())
	...
	for _, log := range receipt.Logs {
		if !strings.EqualFold(log.Address.String(), chainParams.TargetCoreAddr) {
			continue
		}
